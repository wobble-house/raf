...
export default function ProjectModal({id, details, title, image, list, listTitle, link}:{id, details: string, list: { item: string, link: string}[], listTitle: string, link: any, title: string, image:{src:string, alt:string}}){
    ...
    if (isModalOpen) return(
        ...
        >
          <div className={`flex transform justify-center items-center overscroll-contain px-3 pt-3 ${isModalOpen ? "animate-slideUpEnter":"animate-slideUpLeave"}`}>
            <div className="flex flex-col transform duration-300 ease-in-out p-2">
              <button className="absolute justify-self-end right-0 top-0 hover:scale-105 rounded-full bg-red pb-1 z-40 -mt-2 -mr-2" onClick={!isModalOpen ? close : open }>
                <span className="text-xl hover:animate-pulse m-3 text-background font-black">x</span>
              </button>
        ...
        <div id={id} key={`${id} modal closed`} className={`relative flex flex-col overscroll-none border-2 border-primary rounded-lg bg-background group aspect-video hover:text-center overflow-hidden hover:scale-125 z-10 group-hover:z-40 transition-all ease-in-out hover:border-secondary w-80 md:w-60 lg:w-80`} onClick={!isModalOpen ? open : close } style={{cursor: "pointer"}}>
