...
export default function ProjectModal({id, details, gitlink, title, image, list, listTitle, link}:{id, details: string, list: { alt: string, href: string, src: string}[], listTitle: string, gitlink: any, link: any, title: string, image:{src:string, alt:string}}){
    const ref = useRef();
    ...
    return(
      <AnimatePresence>
        ...
        className={`relative group flex flex-col overscroll-none rounded-lg bg-background aspect-video overflow-hidden hover:animate-softwiggle active:animate-hardwiggle z-10 transition-all ease-in-out w-80 md:w-60 lg:w-80 prevent-selection`} onClick={!isModalOpen ? open : close } style={{cursor: "pointer"}}>
