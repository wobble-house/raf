...
export default function ProjectModal({id, details, title, image, list, listTitle, link}:{id, details: string, list: { item: string, link: string}[], listTitle: string, link: any, title: string, image:{src:string, alt:string}}){
    ...
    if (isModalOpen) return(
        ...
        <div id={id} key={`${id} modal closed`} className={`relative group flex flex-col overscroll-none rounded-lg bg-background aspect-video hover:text-center overflow-hidden hover:scale-110 z-10 transition-all ease-in-out w-80 md:w-60 lg:w-80`} onClick={!isModalOpen ? open : close } style={{cursor: "pointer"}}>
